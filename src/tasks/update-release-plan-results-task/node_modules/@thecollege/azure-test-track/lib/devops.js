...
console.log("=== DEVOPS.JS INITIALIZATION ===");
console.log("Organization:", organization);
console.log("Project:", project);
console.log("Token Length:", personalAccessToken?.length || 0);
console.log("Token First 10 chars:", personalAccessToken?.substring(0, 10));
console.log("Token Last 10 chars:", personalAccessToken?.substring(personalAccessToken.length - 10));

const headers = {
  'Authorization': `Basic ${Buffer.from(':' + personalAccessToken).toString('base64')}`
};

console.log("Auth Header (first 50 chars):", headers.Authorization.substring(0, 50));
console.log("Full Auth Header:", headers.Authorization);
...
const getPlanIdByName = async (planName) => {
    ...
    console.log("Base URL:", planUrlBase);
    console.log("Using Auth Header:", headers.Authorization.substring(0, 80) + "...");
    
    try {
      while (true) {
        const url = continuationToken ? `${planUrlBase}&continuationToken=${continuationToken}` : planUrlBase;
        console.log("\n[API CALL] URL:", url);
        console.log("[API CALL] Headers:", JSON.stringify(headers, null, 2));
        ...
        if (typeof response.data === 'string') {
          console.error("[ERROR] Response is HTML/string, not JSON!");
          console.error("[ERROR] First 500 chars:", response.data.substring(0, 500));
          throw new Error("API returned HTML instead of JSON - authentication failed");
        }
        
        console.log("[API RESPONSE] Value Array Length:", response.data.value?.length || 0);
        console.log("[API RESPONSE] First 3 plans:", JSON.stringify(response.data.value?.slice(0, 3), null, 2));
        ...
        if (!continuationToken) {
          ...
          console.error(`Available plans in this call:`, response.data.value.map(p => p.name));
          return null;
        }
      }
    } catch (error) {
        ...
        console.error("Error Data (first 500 chars):", JSON.stringify(error.response?.data).substring(0, 500));
        console.error("URL attempted:", planUrlBase);
        console.error("Organization:", organization);
        console.error("Project:", project);
        console.error("Token Length:", personalAccessToken?.length);
